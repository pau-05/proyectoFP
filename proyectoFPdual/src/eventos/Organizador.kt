...
class Organizador(val nombre: String, val contrasenia: String, val correo: String) {
    ...
    /*aladir método en el main, solo un admin puede añadir admins*/
    //Método para crear perfiles Admins
    fun aniadirAdmin(){
        val nuevoAdmin = Organizador(nombre, contrasenia, correo)
        if (!comprobarAdmin(nombre, contrasenia, correo)){
            arrayAdmins.add(nuevoAdmin)
        }
    }
